...
void FormsManager::createMenu() {
  if (CveManager::instance().boot()->hasRightUser(CAN_SEE_MODULES_MENU)) {
    _modulesMenu  =  CveGui::instance().mainwindow()->menuBar()->addMenu(tr("&Модули"));
    _complexMenu  =  CveGui::instance().mainwindow()->menuBar()->addMenu(tr("&Комплексные окна"));
    createModulesMenu();
  }

  if (CveManager::instance().boot()->hasRightUser(CAN_SEE_MANAGERS_MENU)) {
    _managersMenu =  CveGui::instance().mainwindow()->menuBar()->addMenu(tr("&Менеджеры"));
    createManagersMenu();
  }
}
