...
QAction *mod::getModuleAction_FromWAPair(QString moduleName, QString waName, int id) {
  return Core::ModulesManager::instance().modules()[moduleName].at(id).i->getWidgetActionList()[waName].second;
}
...
QWidget *mod::getModuleWidget_FromWAPair(QString moduleName, QString waName, int id) {
  return Core::ModulesManager::instance().modules()[moduleName].at(id).i->getWidgetActionList()[waName].first;
}
...
QAction *mod::getModuleAction_FromActionsList(QString moduleName, QString actionName, int id) {
  return Core::ModulesManager::instance().modules()[moduleName].at(id).i->getActionList()[actionName];
}
