 ...
 * нагрузку по журналированию берет на себя разработчик модуля или менеджера.
 * Разработчик менеджера или модуля будет обращаться к менеджеру журналирования через
 ...
 * позволяет вести журналировани, но эти функции будут актуальны лишь для текущего менеджера
 * журналирования.
 *
 * \todo При разработке данного менеджера в планах было создать универсальную систему выбора
 * хранилища. Интерфейс хранилища представлен классом ILoggerStorage, который затем
 * переопределяют конкретные хранилища. Необходимо переработать архитектуру менеджера в данной части
 ...
 *
 * \warning Необходимо доработать архитектуру менеджера журналирования до того как
 * демонстрировать код сторонним разработчикам.
 *
 * \date    2015
 * \author  Petrov Gennadii <gennadii.petrov@irz.ru>
 */
...
/*!
 * \brief Класс менеджера журналирования
 */
class LOGGER_EXPORT CveLogger : public QObject, public ILoggerManager {

  Q_OBJECT

  Q_PLUGIN_METADATA(IID ILoggerManager_iid FILE "logger.json")
  Q_INTERFACES(ILoggerManager)
  ...
  CveLogger();
    ...
    /*!
     * \brief Создание виджетов менеджера
     */
    void createWidgets();
